...
$().ready(function() {
	...
	var $specificationValue = $("#specification").find("a");
	...
	$addProductNotify.click(function() {
		...
		if ($addProductNotify.val() == "${message("shop.product.addProductNotify")}") {
			...
			if ($.trim($productNotifyEmail.val()) == "") {
				$.ajax({
					url: "${base}/product_notify/email.action",
					type: "GET",
					dataType: "json",
					cache: false,
	...
	$productNotifyForm.validate({
		...
		submitHandler: function(form) {
			$.ajax({
				url: "${base}/product_notify/save.action",
				type: "POST",
	...
	$addFavorite.click(function() {
		$.ajax({
			url: "${base}/member/favorite/add.action?id=${product.id}",
			type: "POST",
			dataType: "json",
			cache: false,
			success: function(message) {
	...
	<#if setting.isReviewEnabled && setting.reviewAuthority != "anyone">
		...
		$addReview.click(function() {
			if ($.checkLogin()) {
				...
				$.redirectLogin("${base}/review/add/${product.id}.action", "${message("shop.product.addReviewNotAllowed")}");
				return false;
			}
		});
	</#if>
	...
	<#if setting.isConsultationEnabled && setting.consultationAuthority != "anyone">
		...
		$addConsultation.click(function() {
			if ($.checkLogin()) {
				...
				$.redirectLogin("${base}/consultation/add/${product.id}.action", "${message("shop.product.addConsultationNotAllowed")}");
				return false;
			}
		});
	</#if>
	...
	$.ajax({
		url: "${base}/product/history.action",
		type: "GET",
		...
		success: function(data) {
			$.each(data, function (index, product) {
				...
				$historyProduct.append('<li><a href="${base}" target="_blank"><img src="' + thumbnail + '"><span>' + product.name + '</span><b>' + currency(product.price, true) + '<\/b><\/a><\/li>');
			});
		}
	});
	...
	// 点击数
	$.ajax({
		url: "${base}/product/hits/${product.id}.action",
		type: "GET"
	});
	...
	<div class="productTop clear">
		...
		<div class="productInfoBox">
			<div class="productInfo">
				<div class="img_2d">
					<img src="http://item.ule.com/api/ext/generateQRCode?listingId=955944&amp;width=100&amp;height=100">
				</div>
				<div class="name">${product.name}<#if product.isGift> [${message("shop.product.gifts")}]</#if></div>
				<div class="name2">${product.name}<#if product.isGift> [${message("shop.product.gifts")}]</#if></div>
				<div class="info">
					<!-- 货号 -->
					<dl class="sn">
					...
					<div id="specification" class="specification clearfix">
						...
						<#list product.specifications as specification>
							 <dl>
                            	<dt>${abbreviate(specification.name, 8)}:</dt>
					...
					<form id="productNotifyForm" action="${base}/product_notify/save.action" method="post">
					...
					<dl class="quantity">
						...
						<dd class="sale-number">${product.unit!message("shop.product.defaultUnit")}</dd>
					</dl>
					</#if>
				</div>
				<div class="buy">
					...
					<a id="addFavorite" class="addFavorite" href="javascript:;">${message("shop.product.addFavorite")}</a>
				</div>
			</div>
		</div>
	</div>
			...
			<#if (product.parameterValue.size() gte 3)>
				<table class="brief">
					<#list product.parameterValue.keySet() as parameter>
						...
						<#if (parameter_index == 2 && product.parameterValue?size lt 6) || parameter_index == 5>
							</tr>
							<#break />
					...
					<div class="content">
						<@consultation_list productId = product.id count = 5>
							<#if consultations?has_content>
								...
								<p>
									<a href="${base}/consultation/add/${product.id}.action" id="addConsultation">[${message("shop.product.addConsultation")}]</a>
									<a href="${base}/consultation/content/${product.id}.action">[${message("shop.product.viewConsultation")}]</a>
								</p>
							<#else>
								<p>
									${message("shop.product.noConsultation")} <a href="${base}/consultation/add/${product.id}.action" id="addConsultation">[${message("shop.product.addConsultation")}]</a>
								</p>
							</#if>
						</@consultation_list>
					</div>
