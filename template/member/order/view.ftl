...
$().ready(function() {
	...
	$deliveryQuery.click(function() {
		var $this = $(this);
		$.ajax({
			url: "delivery_query.action?sn=" + $this.attr("sn"),
			type: "GET",
			dataType: "json",
			cache: true,
			beforeSend: function() {
		...
		<div class="span18 last">
			<div class="input order">
				...
				<div class="top">
					...
					<span class="handle">
						<#if !order.expired && (order.orderStatus == "unconfirmed" || order.orderStatus == "confirmed") && (order.paymentStatus == "unpaid" || order.paymentStatus == "partialPayment")>
							<a href="payment.action?sn=${order.sn}" class="btn btn-pay">${message("shop.member.order.payment")}</a>
						</#if>
						<#if !order.expired && order.orderStatus == "unconfirmed" && order.paymentStatus == "unpaid">
							<a href="javascript:;" id="cancel" class="btn btn-cancel">${message("shop.member.order.cancel")}</a>
						</#if>
					</span>
					<div class="tips">
						<#if order.expired>
				...
				<table class="info">
					...
					<#if order.promotionDiscount gt 0>
						<tr>
							<th>
