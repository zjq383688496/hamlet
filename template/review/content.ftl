...
<link href="<@website.static />/resources/shop/css/review.css" rel="stylesheet" type="text/css" />
	...
	<#if setting.reviewAuthority != "anyone">
		$addReview.click(function() {
			if ($.checkLogin()) {
				...
				$.redirectLogin("${base}/review/add/${product.id}.action", "${message("shop.review.accessDenied")}");
				return false;
			}
		});
	</#if>
			...
			<table class="info">
				<tr>
					...
					<td class="handle" rowspan="3">
						<a href="${base}/review/add/${product.id}.action" id="addReview">[${message("shop.review.add")}]</a>
					</td>
				</tr>
				<tr>
					<td>
