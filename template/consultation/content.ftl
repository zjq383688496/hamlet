...
<link href="<@website.static />/resources/shop/css/consultation.css" rel="stylesheet" type="text/css" />
	...
	<#if setting.consultationAuthority != "anyone">
		$addConsultation.click(function() {
			if ($.checkLogin()) {
				...
				$.redirectLogin("${base}/consultation/add/${product.id}.action", "${message("shop.consultation.accessDenied")}");
				return false;
			}
		});
	</#if>
			...
			<table class="info">
				<tr>
					...
					<td class="handle" rowspan="3">
						<a href="${base}/consultation/add/${product.id}.action" id="addConsultation">[${message("shop.consultation.add")}]</a>
					</td>
				</tr>
				<tr>
					<td>
